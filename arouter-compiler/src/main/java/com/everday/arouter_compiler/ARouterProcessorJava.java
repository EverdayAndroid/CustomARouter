...
//                    .addStatement("$T.out.println($S)",System.class,"hi wt")
                    .build();

            TypeSpec android = TypeSpec.classBuilder("HelloAPT")
                    .addModifiers(Modifier.PUBLIC)
                    .addMethod(methodSpec)
                    .build();
            JavaFile javaFile = JavaFile.builder("com.ever_day.apt", android).build();
            try {
                javaFile.writeTo(filer);
            } catch (IOException e) {
                e.printStackTrace();
                mess.printMessage(Diagnostic.Kind.NOTE,e.getMessage());
            }
        }
        return true;
    }
